...
-- task_id is a content hash (from|snippet), not a DB-assigned id — the
-- Work/Life category lists are regenerated by the model each run, so
-- persistence keys off the source content instead of a stable row.
CREATE TABLE IF NOT EXISTS swept_done (
  task_id    TEXT PRIMARY KEY,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Same shape, one for business (Work) and one for personal (Life) — both
-- written together by POST /api/report/generate off a single POST /api/pull.
